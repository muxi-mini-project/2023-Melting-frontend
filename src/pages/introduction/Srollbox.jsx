...
export default function Srollbox(props){
    ...
    const [showTaginit,setshowTaginit]=useState(Tags)//所有标签
    const [TagshowKind,setTagshowKind]=useState('personality')
    var showTag=Tags.filter((item)=>{
        return item.type==TagshowKind
    })//根据种类过滤
     


    function changeSelected(tag){
        const newTag=showTaginit.map((item)=>{
            if(item===tag){item.selected=!item.selected;return item}
            else return item
        })
        setshowTaginit(newTag);
    }//选择标签

    return(
        <View className='Srollbox' style={{height:props.height}}>
            <ScrollView className='tagKindbox' scrollX='true'>
                <View className="tagKind" onClick={()=>{setTagshowKind('personality')}}>性格特质</View>
                <View className="tagKind" onClick={()=>{setTagshowKind('interest')}}>兴趣</View>
                <View className="tagKind" onClick={()=>{setTagshowKind('experience')}}>经历</View>
                <View className="tagKind" onClick={()=>{setTagshowKind('game')}}>游戏</View>
                <View className="tagKind" onClick={()=>{setTagshowKind('music')}}>音乐</View>
                <View className="tagKind" onClick={()=>{setTagshowKind('sport')}}>运动</View>
                <View className="tagKind" onClick={()=>{setTagshowKind('activity')}}>线下活动</View>
                <View className="tagKind" onClick={()=>{setTagshowKind('two_dimension')}}>二次元</View>
            </ScrollView>
            <ScrollView className='tagshowbox' scrollY='true'>
                {showTag.map((tag)=>{return<Tag tag={tag} changeSelected={changeSelected}></Tag>})}
            </ScrollView>
